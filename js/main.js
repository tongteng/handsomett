...
require({
    ...
    // 多个js并行加载
    "paths":{
        "jquerys":"http://lib.sinaapp.com/js/jquery/1.8.3/jquery.min.js", //jquery库,如果其他第三方库也可以放在这里
        "default":"http://all.vic.sina.com.cn/hdcommon/js/sina.pc.common.0.2.min.js,/sina.imgslide_3d.js,/page.js" // 这是我们常编辑,修改的参数:js文件路径,可以使绝对路径或者相对路径(需要配置)
    },
    // 多个js同时加载后等待依赖的加载完了后在执行主js,主js需要是一个对象而不是一个方法,因为方法会在js下载完后就直接执行
    // 主js需要等待依赖项加载完后才执行
    "deps":["jQuery"], // 其他js都依赖这个jquery的加载(根据需要配置)
    "callBack" : function(status){
        ...
        status ? require({
            ...
            "deps":["sSina.common","sSina.imgslide_3d","sSina.page"], // 外部js(flash播放的js以及微博登陆的js)依赖我们常修改的js文件
